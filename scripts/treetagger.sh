...
file="$3"/preannotation/TreeTagger/recipes/$filename.words.german
    ...
    sed -i '' -e 's/""/"/g' "$3"/preannotation/TreeTagger/recipes/$filename.words.german
    # replacing * with \*
    cat $file;
    ...
    #rm $filename.tmp
    sed -i '' -e 's/;/POINT_VIRGULE/g' "$3"/preannotation/TreeTagger/recipes/$filename.tagged 
    sed -i '' -e 's/\t/;/g'  "$3"/preannotation/TreeTagger/recipes/$filename.tagged 
    ...
    sed -i '' -e 's/POINT_VIRGULE/";"/g' "$3"/preannotation/TreeTagger/recipes/$filename.treetag_pre-annotation_seed
    ...
    sed -i '' -e 's/$/;10;TreeTagger/g' "$3"/preannotation/TreeTagger/recipes/$filename.treetag_pre-annotation_seed
