  ...
  @foreach($postags as $key=>$postag)
    @if($pos[$postag->id]!='PUNCT')
      @if($postag->difficulty=='easy' || $postag->difficulty=='very-easy' || Auth::user()->hasDoneTutorial($postag->id))
        <div id="postag_{{ $postag->id }}" class="postag list-group-item list-group-item-action {{ $postag->difficulty=='very-easy'?'':'d-none' }}" data-postag-id="{{ $postag->id }}" data-difficulty="{{ $postag->difficulty }}" data-count-todo="">
      ...
      @else
        <div id="postag_{{ $postag->id }}" class="postag list-group-item list-group-item-action disabled warning d-none" data-postag-id="{{ $postag->id }}" data-toggle="tooltip" title="Catégorie difficile, faire la formation ?" data-placement="left" data-difficulty="{{ $postag->difficulty }}" data-count-todo="">
          {{ $postag->full_name }} <em>({{ $postag->name }})</em>
          <i class="float-right fa fa-graduation-cap mr-2" aria-hidden="true"></i>
