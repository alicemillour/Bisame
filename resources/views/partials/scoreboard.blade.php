        ...
        <div class="tab-content">
            <div id="week" class="tab-pane fade in active">
                <div style="margin-top: 5px; display: inline-block;">
                    <span class="score" style="color:black;display: inline-block;">
                        @foreach($users_month as $key=>$user)
                        @if($key == 0)
                        <span  style=" display: inline-block; font-size: 1.3em;"> {{$key + 1}}. {{$user->name}} ({{intval($user->real_score)}}&nbsp;points)
                        <!--</span> <span style="font-size: 0.7em; line-height: 10%">({{intval($user->score * $user->quantity)}}&nbsp; points) </span>--> 
                            @else
                            <span  style="display: inline-block; font-size: 0.7em;"> {{$key + 1}}. {{$user->name}} ({{intval($user->real_score)}}&nbsp;points)
