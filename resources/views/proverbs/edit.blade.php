...
@section('content')
<div class="row">
<div id="create-proverb" class="col-12 col-md-6 offset-md-3 background-proverb">

<h1 class="text-center">{{ __('proverbs.edit-proverb') }}</h1>

{!! Form::model($proverb, ['url' => url('proverbs/'.$proverb->id), 'method' => 'post', 'id' => 'form-proverb']) !!}

<input type="hidden" name="_method" value="PUT">

{!! Form::control('text', 'col-12', 'title', $errors, null, null, null, __('proverbs.title')) !!}

<div class="align-content-start" id="container-ingredients">
	<div class="d-flex">
		<div class="col-6">
			<label for="description" class="control-label">{{ __('proverbs.label-ingredient') }}</label>
		</div>
	</div>

@php
$_ingredients = array();
if(null!==$proverb->ingredients)
foreach($proverb->ingredients as $ingredient){
...
{{-- 		<div class="col-3">
			<input type="text" name="ingredient[{{ $index_ingredient }}][quantity]" class="form-control d-inline quantity {{ ($errors->has('ingredient.'.$ingredient['index'].'.quantity') ? ' is-invalid' : '') }}" value="{{ $ingredient['quantity'] }}" style="width:100%" placeholder="{{  __('proverbs.quantity') }}">
		...
		<div class="col-6">
			<input type="text" name="ingredient[{{ $index_ingredient }}][name]" class="form-control d-inline ingredient-name {{ ($errors->has('ingredient.'.$ingredient['index'].'.name') ? ' is-invalid' : '') }}" value="{{ $ingredient['name'] }}" style="width:100%" placeholder="{{  __('proverbs.ingredient') }}">
...
@empty
	<div class="d-flex row-ingredient">
{{-- 		<div class="col-3">
			<input type="text" name="ingredient[0][quantity]" class="form-control d-inline quantity" value="" style="width:100%" placeholder="{{  __('proverbs.quantity') }}">
		</div>
		<div class="col-auto">
			(de)
		</div> --}}
		<div class="col-6">
			<input type="text" name="ingredient[0][name]" class="form-control d-inline ingredient-name" value="" style="width:100%" placeholder="{{  __('proverbs.ingredient') }}">
		</div>
		<div class="col-auto my-auto">
			<i class="fa fa-plus-circle add-ingredient mr-2" aria-hidden="true" onclick="addIngredient()"></i>
			<i class="fa fa-trash remove-ingredient" aria-hidden="true" onclick="removeIngredient(this)"></i>
...
<div class="collapse mb-1" id="collapseTimes">
	<div class="d-flex">
		<div class="col-3 align-content-start">
			<label for="description" class="control-label">{{ __('proverbs.preparation-time') }}</label>	
		</div>
		<div class="col-3">
			...
			<label for="description" class="control-label">{{ __('proverbs.hours') }}</label>
		...
		<div class="col-3">
			...
			<label for="description" class="control-label">{{ __('proverbs.minutes') }}</label>
	...
	<div class="d-flex">
		<div class="col-3 align-content-start">
			<label for="description" class="control-label">{{ __('proverbs.cooking-time') }}</label>
		</div>
		<div class="col-3">
			...
			<label for="cooking_time_hour" class="control-label">{{ __('proverbs.hours') }}</label>
		...
		<div class="col-3">
			...
			<label for="cooking_time_minute" class="control-label">{{ __('proverbs.minutes') }}</label>
...
<div class="collapse mb-1" id="collapseServings">
	<div class="d-flex">
		<div class="col-3 align-content-start">
			<label for="description" class="control-label">{{ __('proverbs.label-servings') }}</label>
		</div>
		<div class="col-3">
			...
			<label for="servings" class="control-label">{{ __('proverbs.persons') }}</label>
...
<div id="thumbnails">
	@if($proverb->medias)
		@foreach($proverb->medias as $photo)
		<div class="thumbnail ml-3 d-inline-block">
			...
			<label>
				<input type="radio" name="cover_picture" value="{{ $photo->filename }}" {{ ($photo->id == $proverb->thumbnail_id)? 'checked="checked"':'' }}/> Photo de couverture
