...
class AnnotationSeeder extends CsvSeeder {

    public function __construct($filepath)
    {
        $this->table = 'annotations';
        $this->filename = $filepath; //base_path().'/database/seeds/csvs/'. App::getLocale() . '/references.csv';
...
//        DB::table($this->table)->delete();
	...
	{
        Log::debug("seeding annotations for filename " . $filename);
	...
	$row_count = 0;
        ...
        while ( ($row = fgetcsv($handle, 0, $deliminator)) !== FALSE )
        {
            ...
            {        
                ...
                if ( !$row_full )
                    continue; 
                Log::debug("corpus name :" .$row_full['corpus_name']);
